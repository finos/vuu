...
const usa_capital_cities: Record<string, string> = {
  Alabama: "Montgomery",
  Alaska: "Juneau",
  Arizona: "Phoenix",
...
const getCities = (state: string) => {
  if (usa_cities[state]) {
    return usa_cities[state].map((city: string) => ({
      label: city,
    }));
  } else {
    return [];
...
type T = string | WithLabel;

const bySelfOrLabel = (a: T, b: T) => {
  const a1 = (a as WithLabel)?.label ?? a;
  const b1 = (b as WithLabel)?.label ?? b;

  return a1 === b1 ? 0 : a1 > b1 ? 1 : -1;
};

export const groupByInitialLetter = (
  list: string[] | WithLabel[],
  groupMode = "headers-only"
) => {
  ...
  for (const item of sortedList) {
    const label = (item as WithLabel)?.label ?? item;
    if (char !== label[0]) {
