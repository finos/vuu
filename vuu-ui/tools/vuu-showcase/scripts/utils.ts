...
export const execWait = (cmd, cwd = "."): Promise<undefined> =>
...
export const writeFile = async (text, path): Promise<void> =>
  new Promise((resolve, reject) => {
...
export const writeJsonFileFile = async (json, path): Promise<void> =>
  new Promise((resolve, reject) => {
...
export const linkSourceDirectory = (dir: "dev" | "prod") => {
  const cwd = process.cwd();
  const pathToSrc = path.join(cwd, "src");
  const pathToBase = path.join(cwd, `.showcase/${dir}`, "src");

  const { reused, warn = "" } = symlinkDir.sync(pathToSrc, pathToBase);
  console.log(
    `[linkSourceDirectory] pathToSrc ${pathToSrc}, pathToBase ${pathToBase}, reused ${reused} ${warn}`,
  );
};
