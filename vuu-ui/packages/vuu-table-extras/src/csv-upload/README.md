...
| `importSchema` | `TableSchema \| ImportTableSchema` | Schema of the import table, where it differs from the target table. Used to validate the CSV and to determine the session datasource columns. Column definitions in `ImportTableSchema` can explicitly declare some fields as `required: true`. If omitted and `importTable` is provided, the schema is fetched via `getTableSchema`. Pass a stable reference. See [Importing into a separate table](#importing-into-a-separate-table). |
