...
test.describe("WHEN it initially renders, with space for all but one items", () => {
  test("THEN all but one items will be visible, one will be marked as wrapped and overflow Indicator will be visible", async ({
    mount,
  }) => {
    const component = await mount(
    ...
    await expect(wrapContainer).toContainClass(
      "vuuOverflowContainer-wrapContainer-overflowed",
    );
  });
});
