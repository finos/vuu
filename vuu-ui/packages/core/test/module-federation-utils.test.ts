...
describe("module-federation-utils", () => {
  ...
  it("uses the validated portal versions in federation configuration", () => {
    ...
    expect(sharedDependencies["@vuu-ui/vuu-data-test"]).toEqual({
      singleton: true,
      requiredVersion: versions.vuuVersion,
      strictVersion: true,
    });
    ...
    expect(producerSharedDependencies["@vuu-ui/vuu-data-test"]).toEqual({
      singleton: true,
      requiredVersion: versions.vuuVersion,
      strictVersion: true,
    });
