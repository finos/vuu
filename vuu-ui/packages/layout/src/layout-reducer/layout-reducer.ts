...
export const layoutReducer = (state: ReactElement, action: LayoutReducerAction): ReactElement => {
  switch (action.type) {
...
function dragDrop(layoutRoot: ReactElement, action: DragDropAction): ReactElement {
  ...
  if (destinationTabstrip) {
    ...
    newLayoutRoot = dropLayoutIntoContainer(layoutRoot, dropTarget as DropTarget, newComponent);
  }
...
function addChild(layoutRoot: ReactElement, { path: containerPath, component }: AddAction) {
...
function dropLayoutIntoContainer(
  ...
  if (/* existingComponent.path === '0.0' || */ existingComponentPath === '0.0') {
    ...
    var targetContainer = followPathToParent(layoutRoot, existingComponentPath) as ReactElement;
