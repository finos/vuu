...
export const useSplitterResizing = ({ children: childrenProp, onSplitterMoved, style }) => {
  ...
  const handleDragStart = useCallback(
    (index) => {
      const [participants, bystanders] = identifyResizeParties(metaRef.current, index);
      if (participants) {
...
function createPlaceholder(i) {
  return React.createElement(Placeholder, { shim: i === 0, key: `placeholder-${i}` });
}
