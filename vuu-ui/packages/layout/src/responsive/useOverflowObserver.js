...
const addAllExceptOverflowIndicator = (sum, m) => sum + (m.isOverflowIndicator ? 0 : m.size);
...
const measureChildNodes = ({ current: innerEl }, dimension) => {
  const measurements = Array.from(innerEl.childNodes).reduce((list, node) => {
    const {
      collapsible,
      collapsed,
      collapsing,
    ...
    if (index) {
      const size = measureNode(node, dimension);
      if (overflowed) {
...
export default function useOverflowObserver(orientation = 'horizontal', label = '') {
  ...
  useLayoutEffect(() => {
    ...
    if (newlyCollapsed(visibleRef.current)) {
      ...
      if (collapsedItem.fullSize === null) {
        const target = getElementForItem(ref, collapsedItem);
        if (target) {
          const collapsedSize = measureNode(target, dimension);
      ...
      if (target) {
        ...
        const item = {
          ...
          size: measureNode(target, dimension)
        };
