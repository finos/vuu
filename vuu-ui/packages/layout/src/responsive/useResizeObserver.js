  ...
  for (let entry of entries) {
    if (observedMap.has(entry.target)) {
      ...
      for (let [dimension, size] of Object.entries(measurements)) {
        const newSize = isScrollAttribute[dimension] ? entry.target[dimension] : rect[dimension];
        if (newSize !== size) {
          sizeChanged = true;
  ...
  useEffect(() => {
    ...
    async function registerObserver() {
      ...
      if (observedTarget) {
        ...
        console.log(
          `%cuseResizeObserver an target expected to be under observation was not found. This warrants investigation`,
