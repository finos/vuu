...
class AntlrBasedFilterTest extends AnyFeatureSpec with Matchers {
  ...
  Feature("Equals") {
    ...
    Scenario("Equals filter with no index and not first in chain") {
      ...
      val antlrFilter = AntlrBasedFilter(AndClause(Array(EqualsClause("ric", "AAPL.L"), EqualsClause("onMkt", "false"))))
  ...
  Feature("In") {
    ...
    Scenario("In filter with no index and not first in chain") {
      ...
      val antlrFilter = AntlrBasedFilter(AndClause(Array(InClause("ric", Set("AAPL.L")), InClause("onMkt", Set("false")))))
  ...
  Feature("Greater and less than") {
    ...
    Scenario("Greater and less than filter with no index and not first in chain") {
      ...
      val antlrFilter = AntlrBasedFilter(AndClause(Array(GreaterThanClause("tradeTime", "3"), LessThanClause("tradeTime", "6"))))
