...
class JoinsOfJoinsTableTest extends AnyFeatureSpec with Matchers with ViewPortSetup{
  ...
  Scenario("check a tick all the way through from source to join table"){
    ...
    val dateTime = Instant.ofEpochMilli(1437728400000L)

    val ordersDef = TableDef(
      name = "orders",
      keyField = "orderId",
      columns = Columns.fromNames("orderId:String", "trader:String", "ric:String", "tradeTime:EpochTimestamp", "quantity:Double", "ccyCross:String"),
    ...
    ordersProvider.tick("NYC-0001", Map("orderId" -> "NYC-0001", "trader" -> "chris", "tradeTime" -> EpochTimestamp(dateTime), "quantity" -> 100, "ric" -> "VOD.L", "ccyCross" -> "USDGBP"))
    ...
    ordersProvider.tick("NYC-0002", Map("orderId" -> "NYC-0002", "trader" -> "chris", "tradeTime" -> EpochTimestamp(dateTime), "quantity" -> 100, "ric" -> "BT.L", "ccyCross" -> "USDEUR"))
    ...
    val vpcolumns = ViewPortColumnCreator.create(orderPricesFx, List("orderId", "trader", "tradeTime", "quantity", "ric", "fxbid", "fxask", "last"))
    ...
    assertVpEq(filterByVpId(combineQs(viewPort), viewPort)){
      Table(
        ("orderId" ,"trader"  ,"tradeTime","quantity","ric"     ,"fxbid"   ,"fxask", "last"   ),
        ("NYC-0001","chris"   ,EpochTimestamp(dateTime), 100       ,"VOD.L"   ,0.703     ,0.703, Decimal(BigDecimal(221.0004))),
        ("NYC-0002","chris"   ,EpochTimestamp(dateTime), 100       ,"BT.L"    ,1.213     ,1.223, Decimal(BigDecimal(500.0004)))
      )
    }
  }
