...
case class ModuleFactoryNode (tableDefs: TableDefs,
  ...
  def addTable(tableDef: TableDef, func: (DataTable, AbstractVuuServer) => Provider): ModuleFactoryNode = {
    val createDefaultViewPortDefFunc = (table: DataTable, _: Provider, _: ProviderContainer, tableContainer: TableContainer) => ViewPortDef.default(table.getTableDef.getColumns)
