...
class ChildOrdersProvider(val table: DataTable, model: ParentChildOrdersModel)(implicit clock: Clock, lifecycleContainer: LifecycleContainer) extends Provider {
  ...
  model.registerOrderListener(new OrderListener {
    ...
    override def onNewChildOrder(child: ChildOrder): Unit = {processUpsert(child)}
    override def onAmendChildOrder(child: ChildOrder): Unit = {processUpsert(child)}
    override def onCancelChildOrder(child: ChildOrder): Unit = {processUpsert(child)}
  })
