...
object ViewServerModuleBuilder {
  def noMapping(table: DataTable): Provider = null
}

case class ViewServerModuleBuilder(name: String, tableDefs: List[TableDef] = List(), rpcHandler: RpcHandler = null, providerCallback: DataTable => Provider = ViewServerModuleBuilder.noMapping) extends ViewServerModule {
  ...
  def setProvidersCallback(func: DataTable => Provider): ViewServerModuleBuilder = {
