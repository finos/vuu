 ...
 *//*
    ...
    public static void appLogic(GlideClient client) throws ExecutionException, InterruptedException {
        ...
        final String tableName = "bigOrders8";

        HashMap<String, Double> data = new HashMap<>(1);

        for(int i=0; i<100_000_000; i++) {

            data.put("ric:foo" + i + ":cheese:monkey", (double)i);

            client.zadd(tableName, data);
        ...
        Map<String, Double> zAddResult4 = client.zrangeWithScores(tableName, new RangeOptions.RangeByIndex(99_000, 99_100)).get();

        zAddResult4.keySet().stream().toList().stream().sorted().forEach(k -> System.out.println("key->" + k + " value->" + zAddResult4.get(k)));

        //zAddResult4.forEach((x, y) -> System.out.println("X=" + x + " y=" + y));
