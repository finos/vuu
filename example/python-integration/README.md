...
identical Python object, not a generic stub) and ticks `SNAKE_COUNT` rows into it (10,000 by
default, procedurally generated by `_generate_snakes(...)` — random name/type/age/weight per row,
since 10,000 rows isn't practical to write out by hand), passing plain Python `dict`s straight
through with native `int`/`float`/`str` values. Measured against a real JVM, ticking the default
10,000 rows takes ~2s of the process's total startup time.
...
populated with one row per `Snakes` row at startup and re-ticked once a second on a background
Python thread (`threading.Thread(daemon=True)`, no JVM scheduler involved) to simulate movement —
each tick is itself `SNAKE_COUNT` individual JPype calls (~0.4–0.5s measured at the default
10,000), so the loop's actual cadence is closer to ~1.5s than a clean 1Hz; `SnakeLocations`
reflects both tables' columns together, live, for any client subscribed to it.

`Snakes` also exposes a right-click context menu — **"Delete Selected Snake(s)"**, which deletes
the selected row(s) from `Snakes` (`table.processDelete(key)`), and a nested **"Convert To..."**
submenu (a `ViewPortMenuFolder`) with three more items — **Adder**, **Black Mamba**, **Grass
Snake** — that each set the selected row(s)' `type` column to that species via a *partial*
`RowBuilder` update (only `type` is set, so `name`/`age`/`weight` are left untouched). Every
callback is a `SelectionViewPortMenuItem`'s `scala.Function2`, proxied entirely in Python, same
shape as the provider factories; attaching the menu itself needed one small addition to
`vuu-java` — `RpcHandlerBuilder.menu(ViewPortMenu)` — since `RpcHandler` has no abstract methods
for JPype's `@JImplements` to proxy directly (see the RFC addendum for why).
...
`SNAKE_COUNT` procedurally-generated rows (10,000 by default; edit the constant near the top of
`start_server.py` to change it) ticked in from Python at startup, a `Location` table tracking
each snake's `x_coordinate`/`y_coordinate` (populated at startup and moved roughly once a second
thereafter), and a `SnakeLocations` join table combining the two. `Ctrl-C` stops it.
