...
class ParentOrdersService(val table: DataTable, val provider: Provider) extends DefaultRpcHandler with StrictLogging {
  ...
  override def menuItems(): ViewPortMenu = ViewPortMenu("Root",
    ViewPortMenu("Add Rows By Count",
      SelectionViewPortMenuItem("Add 10k Rows", "", this.add10kRows, "ADD_10K_ROWS"),
      SelectionViewPortMenuItem("Add 100k Rows", "", this.add100kRows, "ADD_100K_ROWS"),
      SelectionViewPortMenuItem("Add 1m Rows", "", this.add1mRows, "ADD_1M_ROWS")
    ),
    ViewPortMenu("Add Rows At Rate",
      SelectionViewPortMenuItem("Add 1 row / sec", "", this.add1RowPerSec, "ADD_1ROW_PER_SEC"),
      SelectionViewPortMenuItem("Add 10 rows / sec", "", this.add10RowsPerSec, "ADD_10ROW_PER_SEC"),
      SelectionViewPortMenuItem("Add 100 rows / sec", "", this.add100RowsPerSec, "ADD_100ROW_PER_SEC")
    )
  )
