    ...
    Scenario("Check the creation of the baskets and constituents") {
      ...
      withVuuServer(PriceModule(), BasketModule(omsApi)) {
        vuuServer =>
          ...
          vuuServer.overrideViewPortDef("prices", ViewPortDef.defaultViewPortDefFunc)
          ...
          val rpcResult = basketService.processRpcRequest("createBasket", new RpcParams(Map("sourceBasketId" -> basketId, "basketTradeName" -> "TestBasket"), viewportBasket, vuuServer.requestContext))
          vuuServer.runOnce()
